...
class word2vec():
    ...
    def build_graph(self):
        ...
        with self.graph.as_default():
            ...
            vec_l2_model = tf.sqrt(  # 求各词向量的L2模
                tf.reduce_sum(tf.square(self.embedding_dict),1,keepdims=True)
            )
    ...
    def train_by_sentence(self, input_sentence=[]):
        ...
        for i in range(sent_num):
            ...
            end = min(sent_num,i+self.win_len+1)
            input_id=[]
            for index in range(start,end):
                if index == i:
                    continue
                else:
                    input_id.append(self.word2id.get(input_sentence[index]))
                    if not (input_id and label_id):
                        continue
            label_id = self.word2id.get(input_sentence[i])
